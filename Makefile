...
createdb:
	@docker exec -it postgres18-$(DB_USER)-$(DB_NAME) createdb --username=$(DB_USER) --owner=$(DB_USER) $(DB_NAME)

dropdb:
	@docker exec -it postgres18-$(DB_USER)-$(DB_NAME) dropdb --username=$(DB_USER) $(DB_NAME)

opendb:
	@docker exec -it postgres18-$(DB_USER)-$(DB_NAME) psql -U $(DB_USER) $(DB_NAME)

postgres:
	@docker run --name postgres18-$(DB_USER)-$(DB_NAME) -p 5432:5432 -e POSTGRES_USER=$(DB_USER) -e POSTGRES_PASSWORD=$(DB_PASSWORD) -d postgres:18-alpine
...
##  ─── Redis ───────────────────────────────────────────────────────────────────
redis:
	@docker run -d -p 127.0.0.1:6380:6380 -v ./redis-config:/usr/local/etc/redis --name redis7-$(DB_USER)-$(DB_NAME) redis:7-alpine redis-server /usr/local/etc/redis/redis.conf --aclfile /usr/local/etc/redis/users.acl
...
##  ─── MinIO ───────────────────────────────────────────────────────────────────
minio:
	@docker run -d --name minio-$(DB_USER)-$(DB_NAME) \
